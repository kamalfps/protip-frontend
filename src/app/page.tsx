...
// State for active feature tab simulation
const [activeTab, setActiveTab] = useState<"streamer" | "moderator" | "viewer">("streamer");

// Simulated streaming states for live mock-up dashboard
const [viewerCount, setViewerCount] = useState(1240);
const [isLive, setIsLive] = useState(true);
const [backendStatus, setBackendStatus] = useState<"checking" | "online" | "offline">("checking");
const [pingTime, setPingTime] = useState<number | null>(null);

// Test live connection to the Render backend dynamically
useEffect(() => {
const checkBackend = async () => {
const startTime = Date.now();
try {
const response = await fetch("https://protip-backend.onrender.com/");
if (response.ok) {
...
{/* Background radial effects */}
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-900/80 border border-slate-800 text-xs">
          ...
          <span className="text-slate-400 font-mono">
            API: {backendStatus === "online" ? `Online (${pingTime}ms)` : backendStatus === "offline" ? "Offline" : "Checking..."}
          </span>
        </div>
        
        <button className="hidden sm:inline-flex px-4 py-2 text-sm font-semibold text-white bg-slate-900 border border-slate-800 hover:bg-slate-800 rounded-xl transition">
          Sign In
        </button>
      </div>
    </div>
  ...
  <section className="relative z-10 max-w-7xl mx-auto px-6 pt-16 pb-20 text-center">
    ...
    <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-16">
      <button className="w-full sm:w-auto px-8 py-4 font-bold text-sm bg-gradient-to-r from-purple-600 via-pink-600 to-indigo-600 hover:opacity-95 text-white rounded-xl shadow-lg shadow-purple-500/20 transform hover:-translate-y-0.5 transition active:translate-y-0 flex items-center justify-center gap-2">
        Get Started Free <ArrowRight className="w-4 h-4" />
      </button>
      <a href="#interactive-demo" className="w-full sm:w-auto px-8 py-4 font-bold text-sm bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-300 rounded-xl transition flex items-center justify-center gap-2">
        Explore Dashboard Demo
      </a>
    </div>

    {/* Dynamic Multi-tenant URL Simulator */}
    <div className="max-w-xl mx-auto p-4 rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-lg flex flex-col md:flex-row justify-between items-center gap-4 text-sm shadow-xl">
      <div className="flex items-center gap-2 font-mono text-slate-400">
        <Globe className="w-4 h-4 text-indigo-400" />
        <span>https://</span>
        <span className="text-white font-semibold bg-indigo-500/10 px-2 py-0.5 rounded border border-indigo-500/20">streamer-name</span>
        <span>.protip.live</span>
      </div>
      <span className="text-xs text-slate-500 flex items-center gap-1">
        <CheckCircle className="w-3.5 h-3.5 text-emerald-500" /> Auto DNS Isolation Enabled
      </span>
    </div>
  </section>
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          ...
          <div className="relative aspect-video bg-slate-900 border border-slate-800 rounded-xl overflow-hidden group">
            ...
            <div className="absolute bottom-4 left-4 z-20">
              <p className="text-xs text-purple-400 font-mono font-semibold">WORKSPACE: KAMAL_STREAM</p>
          ...
          {activeTab === "moderator" && (
            <div className="h-full flex flex-col justify-between">
              <div className="space-y-4">
                ...
                <p className="text-xs text-slate-400 leading-relaxed">
                  Moderation boundaries are restricted at DB-level. A moderator bound to tenant <span className="text-white font-mono font-semibold">"kamal"</span> has zero system permissions to cross-access tenant <span className="text-white font-mono font-semibold">"pro-streamer"</span>.
                </p>
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-[10px] bg-slate-950 p-2 rounded border border-slate-900">
                    <span className="text-slate-400">mod_agent_01</span>
                    <span className="text-emerald-400">Authenticated</span>
                  </div>
                  <div className="flex items-center justify-between text-[10px] bg-slate-950 p-2 rounded border border-slate-900">
                    <span className="text-slate-400">mod_agent_02</span>
                    <span className="text-emerald-400">Authenticated</span>
                  </div>
                </div>
              </div>
          ...
          {activeTab === "viewer" && (
            <div className="h-full flex flex-col justify-between">
              <div className="space-y-4">
                ...
                <p className="text-xs text-slate-400 leading-relaxed">
                  Supporters can directly dispatch dynamic financial contributions that routes immediately to your isolated creator node.
                </p>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      ...
      <div className="p-6 rounded-2xl bg-slate-900/30 border border-slate-850 hover:border-slate-800 transition">
        ...
        <p className="text-sm text-slate-400 leading-relaxed">
          Every query is scoped strictly through standard indexes. Your streamers can rest easy knowing there is absolutely zero crossover leakage in records.
        </p>
      </div>
